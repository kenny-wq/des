...
string expand(string bit){      //32->48
	string temp = "";
	for(int i=0;i<48;i++){
...
string single_round(string bit,string round_key){         //64->64  round_key 48
	string temp = bit;
...
void hexstr_to_binarystr(string& s){
	string res = "";
	for(int i=0;i<s.size();i++){
		stringstream ss;
		ss<<hex<<s[i];
...
int main(){
	...
	for(int k=0;k<5;k++){
		cin>>key>>ciphertext;
		str_to_binary(key);
		hexstr_to_binarystr(ciphertext);

		string initial_cipher = initPer(ciphertext);
		...
		string final = sixteen_round(initial_cipher);
		string plaintext = deal_with_final(final);

		vector<int> plaintext_decimal = stringcipher_to_intcipher(plaintext);
		for(int i=0;i<plaintext_decimal.size();i++){
			cout<<(char)plaintext_decimal[i];
		}
		cout<<endl;
	}
	return 0;
